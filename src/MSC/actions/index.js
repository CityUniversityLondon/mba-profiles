...
const fetchPosts = (industry, nationality, programme) => dispatch => {
    ...
    let url = `https://www.cass.city.ac.uk/fb/search.html?form=json&collection=CASS-Student-Profiles&meta_L_orsand=MSC&meta_I_orsand=${i === 'all' ? '': industry}&meta_N_orsand=${n === 'all' ? '': nationality}&meta_P_and=` + pq + '&num_ranks=6&sort=title'
    return fetch(url)
...
export const loadMore = (industry, nationality, programme, page, perPage, totalPages, currEnd) => (dispatch, getState) => {
    ...
    if (perpage < totalPage) {
        let v = perpage + 6
        let url = `https://www.cass.city.ac.uk/fb/search.html?form=json&collection=CASS-Student-Profiles&meta_L_orsand=MSC&meta_I_orsand=${i === 'all' ? '': industry}&meta_N_orsand=${n === 'all' ? '': nationality}&meta_P_and=` + pq + `&num_ranks=${v}&sort=title`
        return fetch(url)
            .then(response => response.json())
            .then(function json(j) { dispatch(receiveProfiles(industry, j));
