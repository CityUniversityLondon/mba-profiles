...
const fetchPosts = (industry, nationality, programme) => dispatch => {
    ...
    if (p === 'all') {
        ...
        pq = '"' + p + '"' + '&meta_P_not="in%20dubai"&meta_P_not="Modular"'
    } else {
        pq = '"' + p + '"'
    }

    let url = `https://www.bayes.city.ac.uk/webservices/funnelback-find-fb16?form=json&collection=city-university~sp-CASS-Student-Profiles&meta_L_orsand=MBA&meta_I_orsand=${i === 'all' ? '': industry}&meta_N_orsand=${n === 'all' ? '': nationality}&meta_P_and=` + pq + '&num_ranks=6&sort=title'
    return fetch(url)
        .then(response => response.json())
        .then(function json(j) {
            dispatch(receiveProfiles(industry, j));
...
export const loadMore = (industry, nationality, programme, page, perPage, totalPages, currEnd) => (dispatch, getState) => {
    ...
    if (perpage < totalPage) {
        let v = perpage + 6
        let url = `https://www.bayes.city.ac.uk/webservices/funnelback-find-fb16?form=json&collection=city-university~sp-CASS-Student-Profiles&meta_L_orsand=MBA&meta_I_orsand=${i === 'all' ? '': industry}&meta_N_orsand=${n === 'all' ? '': nationality}&meta_P_and=` + pq + `&num_ranks=${v}&sort=title`
        return fetch(url)
            .then(response => response.json())
            .then(function json(j) {
                dispatch(receiveProfiles(industry, j));
